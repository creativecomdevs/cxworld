	...
	public static function sql_get_order_ids_Ver1( $settings ) {
		...
		if ( $settings['order_custom_fields'] ) {
			...
			foreach ( $filters as $operator => $fields ) {
				foreach ( $fields as $field => $values ) {
					if ( $values ) {
                                                $left_join_order_meta[] = "LEFT JOIN {$wpdb->prefix}wc_orders AS ordermeta_cf_{$pos} ON ordermeta_cf_{$pos}.order_id = orders.id AND ordermeta_cf_{$pos}.meta_key='$field'";
						if ( $operator == 'IN' OR $operator == 'NOT IN' ) {
							$values              = self::sql_subset( $values );
